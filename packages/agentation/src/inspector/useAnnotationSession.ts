...
import type { InspectedElement, Annotation, AnnotationCategory } from '../types'
 ...
 */
export const CATEGORY_INFO: Record<AnnotationCategory, { label: string; emoji: string }> = {
  bug: { label: 'Bug', emoji: '🐛' },
...
export function useAnnotationSession(
  ...
  const addAnnotation = useCallback(
    (element: InspectedElement, note: string, category: AnnotationCategory): Annotation => {
  ...
  const exportToMarkdown = useCallback((): string => {
    ...
    for (const [category, categoryAnnotations] of byCategory) {
      ...
      for (const annotation of categoryAnnotations) {
        ...
        if (annotation.element.classes.length > 0) {
          lines.push(`- **Classes:** ${annotation.element.classes.join(', ')}`)
        }
        if (Object.keys(annotation.element.dataAttributes).length > 0) {
          lines.push(
            `- **Data attributes:** ${JSON.stringify(annotation.element.dataAttributes)}`
          )
        }
        lines.push('')
        lines.push('---')
